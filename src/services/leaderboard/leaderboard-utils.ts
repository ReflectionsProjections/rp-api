...
export async function getDailyLeaderboard(day: string, n: number): Promise<LeaderboardEntry[]> {
    ...
    const { data: attendees } = await SupabaseDB.ATTENDEES
        .select(`
            userId,
            currentTier,
            icon,
    ...
    const { data: dayEvents } = await SupabaseDB.EVENTS
        .select('eventId, points')
    ...
    const { data: attendances } = await SupabaseDB.EVENT_ATTENDANCES
        .select('attendee, eventId')
        .in('eventId', eventIds)
        .throwOnError();
    ...
    if (attendances) {
        attendances.forEach(attendance => {
            const event = dayEvents.find(e => e.eventId === attendance.eventId);
            if (event && userDailyPoints.has(attendance.attendee)) {
                const currentPoints = userDailyPoints.get(attendance.attendee) || 0;
                userDailyPoints.set(attendance.attendee, currentPoints + event.points);
            }
        });
    }

    // Step 5: Create leaderboard entries and sort
    const leaderboardEntries: Array<LeaderboardEntry & { tempPoints: number }> = attendees.map(attendee => ({
        rank: 0,
 ...
 */
export async function promoteUsersToNextTier(userIds: string[]): Promise<number> {
...
export async function recordLeaderboardSubmission(
    ...
    const { data } = await SupabaseDB.LEADERBOARD_SUBMISSIONS
        .insert({
            day,
            count,
            submittedBy,
