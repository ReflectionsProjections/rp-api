...
async function updateAttendanceRecords(event_id: string, user_id: string) {
	const { data: attendeeAttendance, error: readError } = await SupabaseDB.ATTENDEE_ATTENDANCE
	...
	const { data: eventData } = await SupabaseDB.EVENTS.select("attendance_count")
		.eq("event_id", event_id)
		.single()
		.throwOnError();
